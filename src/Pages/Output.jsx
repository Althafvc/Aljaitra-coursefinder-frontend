...
function Output() {
  ...
  return (
    <div className="wrapper h-screen w-screen">
      ...
      <div
        className="content-wrapper w-screen h-screen flex justify-center items-center"
        style={{
      ...
      >
        <div className="tablecontainer pb-4 w-[85%] h-auto md:w-[80%]   rounded-xl flex flex-col gap-4  bg-[#382388] px-2 lg:max-w-[80%]">
          ...
          <div className="coursename bg-[#008FE8] flex justify-between px-4 md:px-6 items-center py-2 rounded-[6px]">
            {" "}
            <p className="font-semibold  font-kanitFont text-[15px] md:text-[22px] text-white">
              Course Name{" "}
            </p>{" "}
            <p className="font-semibold font-kanitFont text-[15px]   text-white md:text-[22px]">
                ...
                <table className="w-full min-w-max border-collapse border border-white">
                  <thead>
                    <tr className="bg-[#2E3A8F] text-white text-sm md:text-base">
                      {showSpecializations && (
                        <th className="border border-white px-2 md:px-4 py-2 text-left">
                          Specialisations
                        </th>
                      )}
                      {showAdmissionFee && (
                    ...
                    {searchData.FEES_DETAILS &&
                      searchData.FEES_DETAILS.map((fee, index) => (
                        <tr key={index} className="hover:bg-[#4B5BBF]">
                          {showSpecializations && (
                            <td className="border border-white px-2 md:px-4 py-2 text-white text-sm md:text-base font-bold">
                              {fee.specialization}
                            </td>
                          )}
                          {showAdmissionFee && (
